...
import argparse
from nact.utilities import (evaluate_classifier, save_checkpoint_classifier,
                            init_weights_xavier_uniform, count_parameters,
                            detailed_count_parameters)
from nact import scanpy_to_dataloader
from nact import NACTAdditiveModel, NACTProjectionAttention
import pandas as pd
import scanpy
import time
import torch
...
parser.add_argument(
    ...
    type=str,
    default="projection",
    help=("Which N-ACT version we want to use,"
          "default = projection"),
)
...
parser.add_argument(
    ...
    type=str,
    default="",
    help=("The name of dataset that NACT is being trained with,"
...
def main():
    """ Main function for training NACT"""
    ...
    if opt.nact_mode.lower() == "additive-attn":
        mode = "AdditiveAttn"
        model = NACTAdditiveModel(input_dimension=num_genes,
                                  output_dimension=number_of_classes,
                                  device=device).to(device)

    elif opt.nact_mode.lower() == "projection":
        ...
        print(f"==> selected training mode: {opt.NACT_mode}")
        raise ValueError("N-ACT modes are 'additive-attn' or 'projection'."
                         "Please check your entry.")

    if opt.pretrained:
        raise NotImplementedError("Transfer learning options will be released"
                                  " in NACT v0.1.1")
    ...
    # The loss for the task module of NACT.
    ...
    cur_iter = 0
    batch_idx = None
    for ep in tqdm(range(0, opt.epochs), desc=f"N-ACT {mode} Training"):
        batch_losses = []
...
# -----------------------------------------------------------------------------
    ...
    save_checkpoint_classifier(
        ...
        opt.epochs,
        0,
        f"NACT-{mode}-{opt.dataset}-{branching_heads}Branches-",
